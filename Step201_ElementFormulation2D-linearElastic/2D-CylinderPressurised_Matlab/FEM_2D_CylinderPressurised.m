function FEM_2D_CylinderPressurised
% based on [http://solidmechanics.org/FEA.php - FEM_hybrid.m and FEM_volumetric_locking_demo.m]
%
%          Example 2D and 3D Linear elastic FEM code
%          Illustrates use of hybrid elements to model near-incompressible
%          materials
%
...
% Include the "Q1ST_2D_linElastic.m" file for the "Q1ST_2D_linElastic" function
%addpath('../../Step202_ElementFormulation2D-linearElastic-stabilisation')
%
...
% Linear elements
infile=fopen('volumetric_locking_demo_linear.txt','r');
% Quadratic elements (locking free for reduced integration)
% infile=fopen('volumetric_locking_demo_quadratic.txt','r');
...
element_type = "standard_Bower"; % standard_Bower, hybrid_Bower, Q1X, (Q1ST)

%%
% Plot the initial mesh as a check
close all
figure("Name","undeformed mesh (green)")
...
%
  ...
  K = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,element_type);

  r = globaltraction(ncoord,ndof,nnode,nelem,ndload,coords,nelnodes,elident,connect,dloads,dofs,element_type);
...
%
 ...
 figure("Name","deformed mesh (red) vs undeformed (green)")
 plotmesh(coords,ncoord,nnode,connect,nelem,elident,nelnodes,'g');
 hold on
 ...
 b = coords(1,nnode);
 p = 1.;
 E = 2*materialprops(1)*(1+materialprops(2));
 nu = materialprops(2);
 for i =1:nnode
   x1=coords(1,i);
   x2=coords(2,i);
   r=sqrt(x1^2+x2^2);
   ur = (1+nu)*a^2*b^2*p/(E*(b^2-a^2))*(1/r+(1-2*materialprops(2))*r/b^2);
   u1 = ur*x1/(sqrt(x1^2+x2^2));
   u2 = ur*x2/(sqrt(x1^2+x2^2));
   scale = 1.;
   defcoords(1,i) = coords(1,i) + scale*u1;
   defcoords(2,i) = coords(2,i) + scale*u2; 
 end
...
%
function Stif = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,element_type)
%
%   Assemble the global stiffness matrix
%
   npress = numberofpressurepoints(ncoord,nelnodes(1),elident(1),element_type); %TESTING
   Stif = zeros(ndof*nnode+npress*nelem,ndof*nnode+npress*nelem);
...
%
    ...
    switch element_type 
        ...
        case "hybrid_Bower"
            kel = elstif_hybrid(ncoord,ndof,n,ident,lmncoord,materialprops,lmndof,element_type);
        case "Q1X"
            NQP = 1; HG_active = 1;
%             NQP = 4; HG_active = 0;

            % Prepare the input values
            shearMod = materialprops(1);
            nu = materialprops(2);
            bulkMod = (2*shearMod*(1+nu))/(3-6*nu);
            cm = [ 0, 0,        NQP,        HG_active, 0, 0, 0, 0, ...
            ...
            X = lmncoord';
            pe = reshape(lmndof,1,[]);
            % Call the AceGen generated subroutine
            [~,kel,~] = Q1X_2D_linElastic(X,pe,cm,hsve,nhsv,ndtot,istif,ele);

            % If desired add hourglass control, here a simplistic LES (for 2D)
            if ( HG_active==1 )
                [~,stiffHG] = stabilisation_Q1LES_2D(X,lmndof',bulkMod,shearMod,1e-4,istif);
                kel = kel + stiffHG;
            end
%        case "Q1ST"
%            % Prepare the input values
...
%            pe = reshape(lmndof,1,[]);
%            % Call the AceGen generated subroutine
%            [~,kel,~] = Q1ST_2D_linElastic(X,pe,cm,hsve,nhsv,ndtot,istif,ele);
        otherwise
            error("Unknown element_type")
    end
...
%
    for a = 1:nelnodes(lmn)
      for i = 1:ndof
        ...
        for b=1:npress
            rw = ndof*(connect(a,lmn)-1)+i;
            cl = ndof*nnode + npress*(lmn-1)+b;
            Stif(rw,cl) = Stif(rw,cl) + kel(ndof*(a-1)+i,ndof*nelnodes(lmn)+b);
            Stif(cl,rw) = Stif(cl,rw) + kel(ndof*nelnodes(lmn)+b,ndof*(a-1)+i);
        end
      end
    end
    ...
    for a=1:npress
        for b=1:npress
            rw = ndof*nnode + npress*(lmn-1)+a;
            cl = ndof*nnode + npress*(lmn-1)+b;
            Stif(rw,cl) = Stif(rw,cl) + kel(ndof*nelnodes(lmn)+b,ndof*nelnodes(lmn)+a);
        end
    end
   end
end
...
%
function kel = elstif_hybrid(ncoord,ndof,nelnodes,elident,coord,materialprops,displacement,element_type)
%
...
%
   ...
   npress = numberofpressurepoints(ncoord,nelnodes,elident,element_type); % TESTING
   dNdx = zeros(nelnodes,ncoord);
   dxdxi = zeros(ncoord,ncoord);
   strain = zeros(ndof,ncoord);
   kel = zeros(ndof*nelnodes+npress,ndof*nelnodes+npress);
%
...
%
      ...
      for i=1:ncoord
          for j = 1:ncoord
              K = K + dsde(i,i,j,j)/(ncoord*ncoord);
          end
      end
%
        ...
        for i = 1:ndof
          for b = 1:nelnodes
            for k = 1:ndof
              ...
              kel(col,row) = kel(col,row) - K*dNdx(b,k)*dNdx(a,i)*w(intpt)*dt;
            end
          end
        end
      end
...
%
      ...
      if (npress>1) 
          M = shapefunctions(npress,ncoord,elident);
      else
          M(1) = 1.0;
      end
%
%     Compute the terms coupling pressure and displacement
%             
      for a = 1:nelnodes
        for i = 1:ndof
          for b = 1:npress
            row = ndof*(a-1)+i;
            col = ndof*nelnodes + b;
            kel(col,row) = kel(col,row) + M(b)*dNdx(a,i)*w(intpt)*dt;
            kel(row,col) = kel(row,col) + M(b)*dNdx(a,i)*w(intpt)*dt;
          end
        end
      end
%
% Compute the terms coupling pressure
%
      for a = 1:npress
          for b = 1:npress
              row = ndof*nelnodes + a;
              col = ndof*nelnodes + b;
              kel(col,row) = kel(col,row) - M(a)*M(b)*w(intpt)*dt/K;
          end
      end
   end
...
%================= ELEMENT STIFFNESS MATRIX ================================
%
function kel = elstif_standard(ncoord,ndof,nelnodes,elident,coord,materialprops,displacement)
%
%  Assemble the element stiffness
%
%    Arguments;
...
%   each element type
%
function n = numberofreducedintegrationpoints(ncoord,nelnodes,elident)
...
%====================== No. pressure points =============================
%
%   Defines the number of pressure points to be used for
%   each element type
%
function n = numberofpressurepoints(ncoord,nelnodes,elident,element_type)
  
if ( strcmp(element_type, 'hybrid_Bower') )
   if (ncoord == 1) 
     n = nelnodes-1;   
   elseif (ncoord == 2) 
     if (nelnodes == 3)
...
%
function r = globaltraction(ncoord,ndof,nnodes,nelem,ndload,coords,nelnodes,elident,connect,dloads,dofs,element_type)

   npress = numberofpressurepoints(ncoord,nelnodes(1),elident(1),element_type); %TESTING
   r = zeros(ndof*nnodes+npress*nelem,1);
...
%       align_figure(hfig);
% [https://de.mathworks.com/matlabcentral/fileexchange/55468-align-figures-without-overlap]
% 
